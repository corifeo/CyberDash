...
function getAdoptedCount(practices, definitions) {
  ...
  Object.entries(practices).forEach(([key, value]) => {
    ...
    if (!def) return;
    // Skip N/A values - they don't count toward total or adopted
    if (def.type === 'boolean') {
      if (value === 'na') return; // N/A for boolean
      total++;
      if (value) adopted++;
      if (value === def.target) meetsTarget++;
    } else {
      if (value === -1) return; // N/A for maturity
      total++;
      if (value >= 3) adopted++;
...
// Legend Component
function Legend({ darkMode = true, ragColors, orderedPractices = [], maturityScale = [] }) {
